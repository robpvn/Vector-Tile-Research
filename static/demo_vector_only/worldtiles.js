...
var geoJson_layer_railroads = (po.geoJson()
	.url("http://127.0.0.1:8080/edgepointer_trondheim_centre_railroads/{Z}/{X}/{Y}.geojson") //Remember to use the ip adress rather than localhost to avoid XDomain trouble
	.on("load", po.stylist()
		.title(function(d) { return "Name: " + d.properties.name + " ID: " + d.properties.osm_id; })
	)

	.on("load", load_railroads)
///*
	.on("load", tileLoaded)
	.on("added_tile", tileAdded)
	.on("aborted_tile", tileAborted)
...
map.add(geoJson_layer_railroads);
...
var layer_container3 = document.getElementById("org.polymaps.3").parentNode; 
...
function load_railroads(e) {
	for (var i = 0; i < e.features.length; i++) {
		var feature = e.features[i].data, d = feature.properties.osm_id;
		e.features[i].element.setAttribute("class", "railroad");
		e.features[i].element.setAttribute("OSM_id", d);
		if (feature.edgepointer) {
			e.features[i].element.setAttribute("edgepointers", "yes");
			if (feature.edgepointer[0])
...
function concatenateTiles () {
	...
	tiles = layer_container3.lastChild.children;
